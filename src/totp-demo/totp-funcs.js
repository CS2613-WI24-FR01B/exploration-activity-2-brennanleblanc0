const key = "86753099012526132024";
const t_0 = 1711860312690;
...
function totp() {
  ...
  let inputVal = Number(x.elements[0].value);
  let curTime = Date.now()/1000;
  let t = Math.floor((curTime - (t_0 / 1000)) / 30);
  let numHOTP = hotp(key, t);
  let numHOTPOld = hotp(key, t-1);
  let code = numHOTP % 10**6 >= 100000 ? numHOTP % 10**6 : (numHOTP % 10**6) * 10;
  let codeOld = numHOTPOld % 10**6 >= 100000 ? numHOTPOld % 10**6 : (numHOTPOld % 10**6) * 10; 
  if (code == inputVal || codeOld == inputVal) {
    document.getElementById("totp-res").innerHTML = "The code you entered is <b>CORRECT</b>. <br>";
  } else {
    document.getElementById("totp-res").innerHTML = "The code you entered is <b>NOT CORRECT</b>. <br>";
  }
}
